...
describe("getPatchGap", () => {
  it("should get the correct patch difference", async () => {
    expect(getPatchGap(VersionPartsDummyData, "1.0.1", "2.0.1")).toEqual(7);

    expect(getPatchGap(VersionPartsDummyData, "1.0.2", "2.0.1")).toEqual(7);

    expect(getPatchGap(VersionPartsDummyData, "1.5.1", "3.2.1")).toEqual(13);
  });
});

describe("getBannerGap", () => {
  it("should get the correct banner differences", async () => {
    expect(getBannerGap(VersionPartsDummyData, "1.0.1", "1.1.1")).toEqual(1);

    expect(getBannerGap(VersionPartsDummyData, "1.0.1", "1.1.2")).toEqual(2);

    expect(getBannerGap(VersionPartsDummyData, "1.0.1", "2.0.1")).toEqual(14);

    expect(getBannerGap(VersionPartsDummyData, "1.0.2", "2.0.1")).toEqual(13);

    expect(getBannerGap(VersionPartsDummyData, "1.5.1", "3.2.1")).toEqual(25);
  });

  it("should return 0 if current banner is 3.2.1", async () => {
    expect(getBannerGap(VersionPartsDummyData, "3.2.1", "3.2.2")).toEqual(0);
  });

  it("should return -1 if current banner is 3.2.2 to 3.2.2", async () => {
    expect(getBannerGap(VersionPartsDummyData, "3.2.2", "3.2.2")).toEqual(-1);
  });
});
  ...
  it("should return the number of days since last run", () => {
    expect(
      _.orderBy(
        getDaysSinceRunCountSummary(
          VersionPartsDummyData,
          BannerSummariesDummyDataWithFuture,
          "2023-01-03"
        ),
        (b) => b.name,
        "asc"
...
describe("getLongestStatsInBetween()", () => {
  it("should provide longest stats in between", () => {
    expect(
      _.orderBy(
        getLongestStatsInBetween(
          VersionPartsDummyDataWithFuture,
          BannerSummariesDummyDataWithFuture,
          "2023-01-03"
        ),
        (b) => b.name,
...
describe("getCurrentVersionPart", () => {
  ...
  it("should determine latest patch between old banner and new of same patch", () => {
    expect(
      getCurrentVersionPart(
        VersionPartsDummyData,
        BannerSummariesWithOverallDummyData,
        dayjs.utc("2022-11-01")
      )
